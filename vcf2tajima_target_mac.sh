...
# Usage: ./vcf2tajima_target_mac.sh targets.bed Reference.fasta variants.vcf names.args
...
split -l 1 ../"$TARGETS_BED" split_ && for file in split_*; do mv "$file" "$file.bed"; done
...
for i in *.bed; do
    ...
    gatk SelectVariants -V ../"$VARIANTS_VCF" -L "$i" -O "$i.vcf" --exclude-filtered --exclude-non-variants --restrict-alleles-to BIALLELIC --sample-name ../"$NAMES_ARGS"
    ...
    for k in *.vcf; do
        ...
        for l in *.tajima; do
            ...
            m=$(awk '{print $4}' "$l" | grep -Eo '[0-9]+' | sort -rn | head -n 1)
            echo "Maximum value: $m"

            grep -w "$m" "$l" > "${i}_cds.tajimaD"
